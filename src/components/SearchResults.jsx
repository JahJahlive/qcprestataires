...
function SearchResults({ searchPayload }) {
  ...
  const fetchSearchResults = useCallback(
    debounce(async (page) => {
      ...
      try {
        ...
        console.log('SearchResults: Fetching providers with params:', params);
        ...
        console.error('SearchResults: Error fetching providers:', error);
        const errorMessage =
  ...
  }, [pagination.currentPage, fetchSearchResults]);
  ...
  const handleFavorite = useCallback(async (providerId) => {
    try {
      console.log(`SearchResults: Adding provider ${providerId} to favorites`);
    } catch (error) {
      console.error('SearchResults: Error adding favorite:', error);
    }
  }, []);

  const getQuartiersByVille = useCallback((quartiers) => {
    const grouped = quartiers.reduce((acc, { ville, quartier }) => {
      acc[ville.name] = acc[ville.name] || [];
  ...
  return (
    <div className="aon-search-result-area mt-5">
      ...
      <div className="sf-map-filter">
        <button
          className="search-filter-btn btn site-button"
          type="button"
